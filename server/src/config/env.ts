...
export const bootstrapEnv = (): void => {
  ...
  // Helper function: Check individual switch for each resource
  // Returns true if should use PROD, false if should use DEV
  const shouldUseProd = (switchName: string): boolean => {
    const value = process.env[switchName]?.toLowerCase().trim();
    return value === "true" || value === "1" || value === "yes";
  };

  // Helper function: Get value based on individual switch
  const getEnvValue = (switchName: string, key: string, fallback: string = ""): string => {
    ...
    if (useProd && prodValue) {
      return prodValue;
    }
    if (!useProd && devValue) {
      return devValue;
    }
    // Fallback: try prod, then dev, then fallback
    return prodValue || devValue || fallback;
  };

  // Set environment-specific variables based on individual switches
  ...
  // Automatically construct Stripe URLs from FRONTEND_URL (can be overridden)
